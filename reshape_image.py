...
def change_image_shape(ImageDir):
    for image_file in glob.glob( ImageDir + "/*.bmp"):
        image = cv2.imread(image_file)
        h, w = image.shape[:2]
        image ,_ ,_ ,_ ,_ = resize_image(image, max_dim=1024)
        # print(image_file + ".json")
        with open(image_file + ".json","r", encoding='utf-8') as f:
            obj = json.load(f)
        ...
        for anno_position in annos:
            ...
            new_y = np.add(py, 1024//2 - h//2)
            # print(new_y)
            obj["regions"][anno_position]["List_Y"] = new_y.tolist()
        obj["height"] = 1024
        with open(image_file + ".json", "w") as outfile:
...
if __name__ == '__main__':
    ...
    if args.folder:
        change_image_shape(args.folder)